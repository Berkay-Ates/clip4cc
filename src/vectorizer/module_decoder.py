    ...
    def __init__(
        ...
        Args:
            ...
            type_vocab_size: The vocabulary size of the `token_type_ids`\
                passed into `DecoderModel`.
            initializer_range: The sttdev of the truncated_normal_initializer\
                for initializing all weight matrices.
            max_target_embeddings: The maximum sequence length that this model\
...
class BertLMPredictionHead(nn.Module):
    def __init__(self, config, decoder_model_embedding_weights):
        ...
        self.decoder = nn.Linear(
            decoder_model_embedding_weights.size(1),
...
class BertOnlyMLMHead(nn.Module):
    def __init__(self, config, decoder_model_embedding_weights):
        ...
        self.predictions = BertLMPredictionHead(
            config,
            decoder_model_embedding_weights,
        )
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, dec_input, slf_attn_mask=None):
        slf_output, slf_att_scores = self.slf_attn(
            dec_input,
            dec_input,
            dec_input,
            slf_attn_mask,
...
class DecoderLayer(nn.Module):
    ...
    def forward(
        ...
        slf_output, _ = self.slf_attn(
            dec_input,
            dec_input,
            dec_input,
            slf_attn_mask,
        )
        dec_output, dec_att_scores = self.enc_attn(
            slf_output,
            enc_output,
            enc_output,
            dec_enc_attn_mask,
...
class DecoderEmbeddings(nn.Module):
    ...
    def __init__(
        self,
        config,
        decoder_word_embeddings_weight,
        decoder_position_embeddings_weight,
    ):
        super().__init__()
...
class Encoder(nn.Module):
    def __init__(self, config):
        ...
        self.layer = nn.ModuleList(
            [copy.deepcopy(layer) for _ in range(config.num_decoder_layers)],
        )
...
class DecoderModel(PreTrainedModel):
    ...
    def forward(
        ...
        Args:
            ...
            encoder_outs (Tensor, optional): output from the encoder, used for\
        ...
        extended_answer_mask = extended_answer_mask.to(
            dtype=self.dtype,
        )  # fp16 compatibility
