...
class BertAdam(Optimizer):
    ...
    def __init__(
        ...
        if not 0.0 <= warmup < 1.0 and not warmup == -1:
            raise ValueError(
                f"Invalid warmup: {warmup} - should be in [0.0, 1.0[ or -1",
            )
        if not 0.0 <= b1 < 1.0:
            raise ValueError(
                f"Invalid b1 parameter: {b1} - should be in [0.0, 1.0[",
            )
        if not 0.0 <= b2 < 1.0:
            raise ValueError(
                f"Invalid b2 parameter: {b2} - should be in [0.0, 1.0[",
            )
        if not e >= 0.0:
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                # doesn't interact with the m/v parameters. This is equivalent
